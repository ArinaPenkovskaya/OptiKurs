...
<html>
    <head>
        ...
        <title>Оформление депозита</title>
        <link rel="stylesheet" href="css/style.css">
        <link rel="icon" href="assets/icon.png">
    </head>
    <body>
        <sql:setDataSource var="snapshot" driver="com.mysql.jdbc.Driver"
                           url="jdbc:mysql://localhost:3306/optibank?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC"
        ...
        <nav class="header">
            <img class="header__logo" src="assets/bank-logo.png" alt="bank-logo" onclick="location.href = '../OptiKurs'">
        ...
        <section class="content">
            <form class="client-form" action="confD" method="get" >
                <section>
                    ...
                    <div class="client-form__info">
                        ...
                        <input class="client-form__info__text-input" type="text" readonly name="clientID" value="<%= client.getClientID()%>" required >
                ...
                <section>
                    <h2 class="client-form__heading">Информация о депозите</h2>
                    <div class="client-form__info">
                        <p class="client-form__info__text">Депозит*</p>
                        <c:forEach var="row" items="${result1.rows}">
                            <c:set var="a" scope="session" value="${row.depositID}"/>
                            <c:set var="b" scope="session" value="<%= od.getDepId()%>"/>

                            <c:if test="${a == b}">
                                <input class="client-form__info__text-input" type="text" readonly name="depName" value="<c:out value="${row.depositName}"/>" required >
                            </c:if>
                        ...
                        <input class="client-form__info__text-input" type="text" readonly name="percent" value="<%= od.getPercent()%>" required >
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div class="client-form__info">
                        <p class="client-form__info__text">Дата открытия*</p>
                        <input class="client-form__info__text-input" type="text" readonly name="dateOfStart" value="<%= od.getDateOfStart()%>" required >
                        <p class="client-form__info__text">Дата закрытия*</p>
                        <input class="client-form__info__text-input" type="text" readonly name="dateOfFinish" value="<%= od.getDateOfFinish()%>" required >
                        <p class="client-form__info__text">Срок вклада*</p>
                        <input class="client-form__info__text-input" type="text" readonly name="period" value="<%= od.getPeriod()%>" >
                    </div>
                </section>

                <section>
                    <h2 class="client-form__heading">Размер вклада</h2>
                    <div class="client-form__info">
                        <p class="client-form__info__text">Сумма вклада*</p>
                        <input class="client-form__info__text-input" type="text" readonly name="summa" value="<%= od.getSumma()%>" required >
                        <p class="client-form__info__text">Проценты по окончанию срока*</p>
                        <input class="client-form__info__text-input" type="text" readonly name="accumulation" value="<%= od.getAccumulation()%>" required >
                    </div>
                </section>

                <button class="client-form__button content__button">Открыть депозит</button>
            </form>
        </section>
    </body>
</html>
